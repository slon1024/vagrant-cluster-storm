...
for file in /usr/share/java/mx4j-tools.jar;
do
  CLASSPATH=$CLASSPATH:$file
done
...
if [ -z "$KAFKA_JMX_OPTS" ]; then
  KAFKA_JMX_OPTS="-Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.authenticate=false  -Dcom.sun.management.jmxremote.ssl=false -Dmx4jport=8083 -Dmx4jaddress=0.0.0.0 -Dkafka_mx4jenable=true"
fi
